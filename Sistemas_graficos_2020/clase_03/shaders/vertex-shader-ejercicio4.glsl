        ...
        void main(void) {
           ...
           vec4 textureColor = texture2D(uSampler, vec2(uv.s, uv.t));         
            ...
            // Ejercicio 4: agua con movimiento de ondas + terreno elevado

            if (textureColor.x <  0.04) {              
                // agua => movimiento ondas
                position *= 1. + 0.02*cos(uv.s*2.*PI*10.+time*20.)*sin(uv.t*2.*PI*10.+time*20.);
            } else {
                // terreno => elevación
                position *= 1.06;
            }
