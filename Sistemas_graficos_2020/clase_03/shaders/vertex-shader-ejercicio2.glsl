        ...
        void main(void) {
           ...
           vec4 textureColor = texture2D(uSampler, vec2(uv.s, uv.t));         
            ...
            // Ejercicio 2: efecto de onda en función a la distancia al centro
            
            float dist = sqrt(pow(uv.s-0.5, 2.0)+pow(uv.t-0.5, 2.0));
            float maxHeight = pow(0.6-dist, 4.0);
            float height = maxHeight*(cos(70.0*(time*0.5-dist)));

            position = vec3(uv.s-0.5, height, uv.t-0.5);
