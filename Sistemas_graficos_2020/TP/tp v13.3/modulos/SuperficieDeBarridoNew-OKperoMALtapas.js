...
(function(){
    ...
    function SuperficieDeBarridoNew(filas, columnas) {
        ...
        this.getBuffers = function(){
            ...
            var nivelInicio = 0;
            if (_bHasTapaBottom) nivelInicio = 2;
            ...
            if (_bHasTapaBottom) {
                ...
                for (var j = 0; j <= _columnas; j++) { // nivel 0 = 0 a 10 / nivel 2 = 11 a 21

                    // reserva de lugares para centro tapa en el buffer - nivel 0
                ...
                // si hay tapaBottom, nivel 1 y 2 son iguales, varian las normales (falta)
                for (var n = 1; n < 3; n++) {
            ...
            // console.log("pos buffer length", positionBuffer.length); // para cols=10 > 33
            // console.log("uv buffer length", uvBuffer.length);        // para cols= 10 > 22

            for (var i = nivelInicio+1; i < niveles+nivelInicio; i++) {
            ...
            if (_bHasTapaBottom) {
                ...
                for (var j = 0; j < _columnas; j++) {
                    ...
                    // sale raro el promedio, desbalanceado a origen
                    var index = _columnas + 1 + j*3;
                    // puntos
            ...
            if (_bHasTapaTop) {
                ...
                for (var j = 0; j <= _columnas; j++) {
                    ...
                    var n = (niveles + nivelInicio- 1);

                    var index = (_columnas+1) * n + j*3;
                    // puntos
                    x += positionBuffer[index];
                    y += positionBuffer[index+1];
                    z += positionBuffer[index+2];
                    ...
                    positionBuffer.push(positionBuffer[index]);
                    positionBuffer.push(positionBuffer[index+1]);
                    positionBuffer.push(positionBuffer[index+2]);

                    normalBuffer.push(normalBuffer[index]);
                    normalBuffer.push(normalBuffer[index+1]);
                    normalBuffer.push(normalBuffer[index+2]);

                    uvBuffer.push(j/_columnas);
                    uvBuffer.push(n/(niveles+nivelInicio));

                }
                
                var taver = vec3.fromValues(x/count, y/count, z/count);               
                var tnaver = vec3.fromValues(nx/count, ny/count, nz/count);    
                                

                for (var j = 0; j <= _columnas; j++) {

                    positionBuffer.push(taver[0]);
                    positionBuffer.push(taver[1]);
                    positionBuffer.push(taver[2]);

                    normalBuffer.push(tnaver[0]);
                    normalBuffer.push(tnaver[1]);
                    normalBuffer.push(tnaver[2]);
            ...
            // console.log("pos buffer length", positionBuffer.length); // para cols=10 > 132
            // console.log("uv buffer length", uvBuffer.length);        // para cols= 10 > 88
